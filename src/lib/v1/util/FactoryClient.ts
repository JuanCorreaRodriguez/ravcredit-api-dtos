...
export class FactoryClient {
    ...
    public static async CreateClientObj(
        data: ClientDto | PartialClientDto,
        action: eGCActions,
    ) {
        const o: any = {};
        ...
        if (data.curp) o['curp'] = data.curp;

        if (data.contract) o['contract'] = data.contract;

        if (data.conekta_id) o['conekta_id'] = data.conekta_id;

        if (data.payments) o['payments'] = data.payments;

        if (data.reference) o['reference'] = data.reference;

        if (data.userReferences) o['userReferences'] = data.userReferences;

        if (data.contractUrl) o['contractUrl'] = data.contractUrl;

        if (data.status) o['status'] = data.status;

        if (data.dynamic_id) o['dynamic_id'] = data.dynamic_id;

        if (data.dynamic_account) o['dynamic_account'] = data.dynamic_account;
    ...
    public static async EncryptPassword(data: any) {
        const o: any = {};
        // Encrypt Password
        if (data['password'])
            o['password'] = await Crypter.encryptPassword(data.password);
    ...
    //Guard Type
    public static checkClientType(object: any): object is PartialClientDto {
        return 'conekta' in object;
    }

    public static CreateClientFromDynamicObj(account: IDCDataAccountDto, client: IDCClientDto): ClientDto {
        ...
        if (client.id) o.dynamic_id = String(client.id);
        if (account.id) o.dynamic_account = String(account.id);
